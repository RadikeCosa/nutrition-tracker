...
describe("RegistrationForm - Rendering", () => {
  ...
  it("renders all required form fields", () => {
    ...
    expect(screen.getByLabelText(/tipo de comida/i)).toBeInTheDocument();
  ...
  it("renders optional fields", () => {
    ...
    expect(screen.getByLabelText(/endulzante/i)).toBeInTheDocument();
  ...
  it("renders unit select with default value 'unit'", () => {
    render(<RegistrationForm />);
  ...
  it("renders mealType select with default value 'breakfast'", () => {
    ...
    const mealTypeSelect = screen.getByLabelText(
      /tipo de comida/i
    ) as HTMLSelectElement;
    expect(mealTypeSelect.value).toBe("breakfast");
  });

  it("renders sweetener select with 'Sin endulzante' option", () => {
    render(<RegistrationForm />);

    const sweetenerSelect = screen.getByLabelText(
      /endulzante/i
    ) as HTMLSelectElement;
    const options = Array.from(sweetenerSelect.options);

    // Buscar la opción con valor vacío o "null"
    const noSweetenerOption = options.find(
      (opt) => !opt.value || opt.value === "null"
    );
    expect(noSweetenerOption).toBeDefined();
  });
